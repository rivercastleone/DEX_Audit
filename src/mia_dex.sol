...
contract Dex is IDex {
    ...
    modifier refresh() {
        uint256 bx = tokenX.balanceOf(address(this));                                                   // bx, by => 변경된 토큰량 | balanceX, balanceY => 기존 토큰량
        uint256 by = tokenY.balanceOf(address(this));                                                  
        dx = bx != 0 ? bx > balanceX ? int256(bx - balanceX) : -1 * int256(bx - balanceX) : int256(0); // 1. bx 값이 0이면 0 반환
        dy = by != 0 ? by > balanceY ? int256(by - balanceY) : -1 * int256(by - balanceY) : int256(0); // 2. bx가 balanceX보다 큰 경우 (bx - balanceX)를 반환 | bx가 balanceX보다 작은 경우 -(bx - balanceX)를 반환 | by도 과정은 동일
        balanceX = uint256(int256(balanceX) + dx);                                                     // balanceX 업데이트 => 기존 토큰량 balanceX + 반환된 값
        balanceY = uint256(int256(balanceY) + dy);                                                     // balanceY 업데이트 => 기존 토큰량 balanceY + 변환된 값     
        _;
    }
        ...
        returns (uint256 lpAmount)
    {
        ...
        require(balanceX / amountX == balanceY / amountY,"error this");
